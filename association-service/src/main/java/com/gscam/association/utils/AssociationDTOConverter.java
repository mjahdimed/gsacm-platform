...
import com.gscam.association.models.Address;
import com.gscam.association.models.Association;
import com.gscam.association.models.Info;
...
public class AssociationDTOConverter {
    ...
    public static AssociationDTO fromEntity(Association association) {
        ...
        // Retourn les propriétés créer associationDTO
        return AssociationDTO.builder()
                .id(association.getId())
                .name(association.getName())
                .logoUrl(association.getLogoUrl())
                .description(association.getDescription())
                .email(association.getInfo().getEmail())
                .numFix(association.getInfo().getNumFix())
                .numFax(association.getInfo().getNumFax())
                .gsm(association.getInfo().getGsm())
                .siteWeb(association.getInfo().getSiteWeb())
                .address1(association.getAddress().getAddress1())
                .address2(association.getAddress().getAddress2())
                .ville(association.getAddress().getVille())
                .codepostale(association.getAddress().getCodepostale())
                .pays(association.getAddress().getPays())
                .status(association.getStatus())
                .creationDate(association.getCreationDate())
                .lastModifiedDate(association.getLastModifiedDate())
                .deletedDate(association.getDeletedDate())
                .build();
    }
    ...
    public static Association toEntity(AssociationDTO associationDTO) {
        // Vérifier si associationDTO est vide
        ...
        association.setLogoUrl(associationDTO.getLogoUrl());
        ...
        Info info = new Info();
        info.setEmail(associationDTO.getEmail());
        info.setNumFix(associationDTO.getNumFix());
        info.setNumFax(associationDTO.getNumFax());
        info.setGsm(associationDTO.getGsm());
        info.setSiteWeb(associationDTO.getSiteWeb());
        association.setInfo(info);

        // Nouvelle instance Adresses entité
        Address address = new Address();
        address.setAddress1(associationDTO.getAddress1());
        address.setAddress2(associationDTO.getAddress2());
        address.setVille(associationDTO.getVille());
        address.setCodepostale(associationDTO.getCodepostale());
        address.setPays(associationDTO.getPays());
        association.setAddress(address);

        // Définir des propriétés à partir de la classe AbstractEntity intégrées
        association.setCreationDate(associationDTO.getCreationDate());
        association.setLastModifiedDate(associationDTO.getLastModifiedDate());
        association.setDeletedDate(associationDTO.getDeletedDate());
